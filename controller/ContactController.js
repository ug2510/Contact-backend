...
export const getDeletedContacts = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500);
    throw new Error(err.message || "Failed to fetch deleted contacts.");
  }
});
...
export const getContactById = asyncHandler(async (req, res) => {
  try {
    ...
    throw new Error(err.message || "Failed to fetch contact by name.");
  }
});
...
export const getPhnumberById = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(err.statusCode || 500);
    throw new Error(err.message || "Failed to fetch contact by phone number.");
  }
});
...
export const createContactUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (!name || !email || !phnumber || !address) {
      res.status(400);
      throw new Error("Missing required fields: name, email, phnumber, and address.");
    }
    ...
    res.status(err.statusCode || 500);
    throw new Error(err.message || "Error creating contact user.");
  }
});
...
export const deactivateContactById = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(err.statusCode || 500);
    throw new Error(err.message || "Error deactivating contact.");
  }
});
...
export const updateContact = asyncHandler(async (req, res) => {
  try {
    ...
    if (!name && !email && !newPhone) {
      return res.status(400).json({
        success: false,
        message: "At least one field (name, email, or phone number) must be provided for the update.",
      });
    }

    if (newPhone && (!/^\d+$/.test(newPhone) || newPhone.length !== 10)) {
      res.status(400);
      throw new Error("Phone number must be a valid 10-digit integer.");
    }

    if (name && !/^[a-zA-Z]+(\s[a-zA-Z]+)*$/.test(name.trim())) {
      res.status(400);
      throw new Error("Name must only contain letters and should not be empty or just spaces.");
    }
    ...
    const result = await updateContactByPhone(oldPhone, updatedName, updatedEmail, updatedPhone);
    if (result && result.rowsAffected[0] > 0) {
      res.status(200).json({
        success: true,
    ...
    res.status(err.statusCode || 500);
    throw new Error(err.message || "Error updating contact.");
  }
});
